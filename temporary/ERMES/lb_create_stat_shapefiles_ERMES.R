...
for (cc in c('gr','es','it')) {
  ...
  in_rast_ricemap_dir = '/home/lb/projects/ermes/datasets/ERMES_Folder_Structure/IT/Regional/IT_EP_R1_Rice_Map/2016/Late/'
  in_rast_sow_dir = '/home/lb/projects/ermes/datasets/ERMES_Folder_Structure/IT/Regional/IT_EP_R2_Phenology/2016/MinDoys/'
  in_rast_flw_dir = '/home/lb/projects/ermes/datasets/ERMES_Folder_Structure/IT/Regional/IT_EP_R2_Phenology/2016/MaxDoys/'
  out_filename_in =  "/home/lb/Temp/buttami/FOR_Stats_Shapefiles/Regional/IT_Statistics_Shapefile_2016.shp"
  dir.create(dirname(out_filename_in), recursive = TRUE)
  ...
  in_shp = lb_get_PGIS(schema_name, table_name, con, geom_field = 'geom',verbose = T)
  ...
  # in_rast_map = "D:/Documents/ERMES/Documents/Data_Processing/FOR_Stats_Shapefiles/IT_Ricemap_Full_2016_254.tif"
  in_rast_ricemap = raster(in_rast_ricemap)
  in_rast_flw = raster(in_rast_flw)
  in_rast_sow = raster(in_rast_sow)

  in_rast_area = setZ(stack(in_rast_ricemap), as.Date('2016-12-31'), name = 'time')
  ...
  in_rast_sow = setZ(stack(in_rast_sow), as.Date('2016-12-31'), name = 'time')
  ...
  in_rast_flw = setZ(stack(in_rast_flw), as.Date('2016-12-31'), name = 'time')
  ...
  if (cc == "es") {
    ...
    out = in_shp@data %>%
      ...
      left_join(fc) %>%
      left_join(avgsow) %>%
      mutate(avgsow = doytodate(avgsow, 2016)) %>%
      left_join(avgflw) %>%
      mutate(avgflw = doytodate(avgflw, 2016)) %>%
  ...
  writeshape(out_shp,out_filename,overw = TRUE)
