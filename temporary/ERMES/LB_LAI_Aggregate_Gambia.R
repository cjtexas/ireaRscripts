...
in_raster_file = 'D:/Documents/ERMES/Documents/Study Areas/WA/GM/Examples/vgt_input.dat'
in_lc_file = '//10.0.1.252/nr_working/shared/PhenoRice/Processing/Gambia/LC_Map/lc_map_GM_NOwat_and_dense.dat'
in_nodata = 255
in_raster_proj = 'Geo_WGS84'
in_lc_raster_proj = CRS('+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181 +units=m +no_defs')
in_ermes_grid_laea  = 'D:/Documents/ERMES/Documents/Study Areas/WA/GM/gm_2km_grid_laea.shp'
out_folder = 'D:/Documents/ERMES/Documents/Data_Processing/EO_Data_Aggregation/GM/VGT'
out_filename = 'D:/Documents/ERMES/Documents/Data_Processing/EO_Data_Aggregation/GM/VGT_test'
out_format = 'GTiff'
fc_threshold = 0.75

# Define Proj4 strings for warping
if (in_raster_proj == 'Geo_WGS84') {in_raster_file_proj4='+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0,0,0,0'}
if (in_raster_proj == 'MODIS_SIN') {in_raster_file_proj4='+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181 +units=m +no_defs'}
...
out_file_raster = file.path(out_folder_raster, paste(basename(file_path_sans_ext(in_raster_file)),'_LAEA', sep = ''))
...
laea_crs = CRS('+proj=laea +lat_0=52 +lon_0=10 +x_0=4321000 +y_0=3210000 +ellps=GRS80 +units=m +no_defs')
...
if(file.exists(out_ermes_grid_reproj) == FALSE) {
  ...
  writeOGR(ERMES_cells_poly_reproj,dsn = dirname(out_ermes_grid_reproj), layer =file_path_sans_ext(basename(out_ermes_grid_reproj)),driver="ESRI Shapefile", overwrite_layer = T) #save the shapefile
...
extent_poly_reproj = spTransform(extent(ERMES_cells_poly_reproj), laea_crs)  # Retrieve extnt of ERMES grid in the projection of input raster
...
# Create a temporary raster file with higher spatial resolution than the original, resized on the extent of the ERMES reference grid
gdalwarp(out_temp_raster_masked,file.path(temp_folder, 'temporary_raster.tif'), s_srs = raster_orig_crs, t_srs = laea_crs,
         tr = temp_res, r = "near", te = c(extent_poly_reproj@xmin, extent_poly_reproj@ymin,extent_poly_reproj@xmax, extent_poly_reproj@ymax),
         srcnodata = in_nodata, dstnodata =  in_nodata,  overwrite = T, tap = F, output_Raster = T)
...
if (file.exists(file.path(temp_folder, paste('ERMES_Grid_Rasterized_',in_raster_proj,'.tif', sep = ''))) == FALSE){
  gdal_rasterize(out_ermes_grid_reproj,file.path(temp_folder, paste('ERMES_Grid_Rasterized_',in_raster_proj,'.tif', sep = '')),a = "int_id", output_Raster = T,
                 te = c(extent_poly_reproj@xmin, extent_poly_reproj@ymin,extent_poly_reproj@xmax, extent_poly_reproj@ymax), tr = temp_res, tap = F)
}
# Get the values of the "dummy" raster of cells codes
temp_raster_cells <- raster(file.path(temp_folder, paste('ERMES_Grid_Rasterized_',in_raster_proj,'.tif', sep = '')))
