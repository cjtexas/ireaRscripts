...
lb_compute_pheno_stats = function(cc,in_rast_ricemap = NULL,in_rast_flw = NULL, in_rast_sow = NULL , out_folder = NULL, out_shapefile = out_shapefile, sow = NULL, flw = NULL, area = NULL , con = con){
  ...
  avgsow = lb_fastzonal(in_rast_sow, in_shp, id_field = "adm_id", out_format = 'dframe', verbose = F)
  ...
  avgflw = lb_fastzonal(in_rast_flw, in_shp, id_field = "adm_id", out_format = 'dframe', verbose = F)
  ...
  out = avgsow  %>%
    ...
    mutate(anomsow = avgsow - mean(avgsow, na.rm = TRUE)) %>%
    mutate(avgflw_date = lb_doytodate(avgflw, year)) %>%
    mutate(anomflw = avgsow - mean(avgsow, na.rm = TRUE)) %>%
