...
#' @examples
#'
fastzonal = function(in_rts, sp_object, start_date = NULL, end_date = NULL,
                     start_band = NULL, end_band = NULL,
                     ...
                     small_method = "centroids", na.rm = TRUE, verbose = FALSE)
{
  ...
  if (class(sp_object) %in% c("SpatialPolygonsDataFrame",
    ...
    sp_type <- ifelse(class(sp_object %in% c("RasterLayer", "RasterStack", "RasterBrick")), "rastobject", "spobject")
  } else {
    if (class(sp_object) == "character") {
      ...
      if (class(zone_object) == "try-error") {
        ...
        if (class(zone_object) == "try-error") {
          ...
          stop("sp_object is not a valid Spatial object, Shapefile or Raster file - please check !")
        } else {
          sp_type = "rastobject"
        }
      } else {
  ...
  if (!ts_check) {dates <- seq(1, nlayers(in_rts),1)}
  ...
  if (length(sel_dates) > 0) {
    ...
    if (sp_type == "spobject") {
      ...
      if (class(shape) %in% c("SpatialPointsDataFrame", "SpatialPoints",
        ...
        message("On point and lines shapefiles, the standard `extract` function is used. This could be slow !")

        ts <- matrix(nrow = length(sel_dates), ncol = length(shape[,1]))
        for (f in 1:length(sel_dates)) {
          if (verbose == TRUE) {
            message(paste0("Extracting data from ", ifelse(ts_check, "date: ", "band: "), dates[sel_dates[f]]))
          }
        ...
        if (max(shape@data$mdxtnq) <= 255) {ot = "Byte"}  else
        {if (max(shape@data$mdxtnq) <= 65536) {ot = "Int16"} else {ot = "Int32" }
        }
        gdal_rasterize(tempshape, tempraster, tr = raster::res(in_rts), te = extent(in_rts)[c(1,3,2,4)],
                       a = "mdxtnq", ot = ot)
